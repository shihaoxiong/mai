...
class RuntimeMcpToolProviderTests {
    ...
    @Test
    void shouldLoadAndExecuteRealSseMcpToolFromRuntimeConfig() {
        try (TestHttpMcpServer server = TestHttpMcpServer.start()) {
            RuntimeConfigProperties properties = new RuntimeConfigProperties();
            properties.setFile(tempDir.resolve("runtime-config-sse.json"));
            FileRuntimeConfigRepository repository = new FileRuntimeConfigRepository(properties, new ObjectMapper());
            McpConfigService mcpConfigService = new McpConfigService(repository);
            ...
            try {
                ...
                assertThat(tools).hasSize(1);
                assertThat(tools.get(0).getToolDefinition().name())
                        .isEqualTo(RuntimeMcpToolCallback.exposedName("demo-sse", "mcp_reverse"));
                assertThat(tools.get(0).call("{\"text\":\"beta\"}")).isEqualTo("MCP:ateb");
            }
            finally {
                provider.close();
    ...
    @Test
    void shouldLoadAndExecuteRealStreamableHttpMcpToolFromRuntimeConfig() {
        try (TestHttpMcpServer server = TestHttpMcpServer.start()) {
            RuntimeConfigProperties properties = new RuntimeConfigProperties();
            properties.setFile(tempDir.resolve("runtime-config-http.json"));
            FileRuntimeConfigRepository repository = new FileRuntimeConfigRepository(properties, new ObjectMapper());
            McpConfigService mcpConfigService = new McpConfigService(repository);
            ...
            try {
                ...
                assertThat(tools).hasSize(1);
                assertThat(tools.get(0).getToolDefinition().name())
                        .isEqualTo(RuntimeMcpToolCallback.exposedName("demo-http", "mcp_reverse"));
                assertThat(tools.get(0).call("{\"text\":\"beta\"}")).isEqualTo("MCP:ateb");
            }
            finally {
                provider.close();
